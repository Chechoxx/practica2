...
#define EXAMPLE_MQTT_SERVER_HOST "192.168.1.103"
...
{
    static const char *topics[] = {"sensor/DHT11/temperature", "sensor/DHT11/humidity","sensor/water_level"};
    int qos[]                   = {0, 1, 2};
    err_t err;
    int i;
...
static void publish_message(void *ctx)
{
	...
	static const char *topic   = "alarma";
    static const char *topic1   = "motor";
    deb = rand()%101;

    char *message;
    char *message1;
    //niveles de ruido por encima de 85 dB son malos, se pone 80 solamente para verlo con más facilidad
    //static const char *message = "Saludos de la Freedom K64 a MQTT";
    PRINTF("Esto vale el ruido \"%d\"...\r\n", deb);
    int longitud;
    //alarma="0";
    //motor="0";
    if (deb>80){
    	PRINTF("Alarma de contaminacion auditiva alta! \r\n");
    	message="1";
    	LED_RED_ON();
    	LED_BLUE_ON();
    	LED_GREEN_ON();
    ...
    PRINTF("Publicando en el Topic \"%s\"...\r\n", topic);
    ...
    mqtt_publish(mqtt_client, topic1, message1, strlen(message1), 1, 0, mqtt_message_published_cb, (void *)topic);
    //mqtt_publish(mqtt_client, topic1, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}
